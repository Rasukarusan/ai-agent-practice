...
| 種類 | 保存の性質 | 想定される保存先 |
|------|-----------|----------------|
| **Semantic（意味記憶）** | 事実をベクトル化して検索 | OpenSearch（`memories` インデックス）← 今回実装 |
| **Episodic（エピソード記憶）** | 成功体験を Few-shot 例として保存 | OpenSearch（別インデックス）or RDB |
| **Procedural（手続き記憶）** | システムプロンプト自体を書き換え | ファイル or DB（ベクトル検索は不要） |
